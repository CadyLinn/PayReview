...
| 財務計算 | 獨立 Swift `FinanceEngine` | 預算、目標日期、延後天數與恢復方案 |
...
- 評估：快速金額輸入、預算與目標日期影響、超出預算警告、購買或延後選項。
...
- 計算目標剩餘時間、最低儲蓄速度、無恢復情境的延後日期與天數，以及恢復方案。
...
6. 建立獨立 `FinanceEngine` 及金額、日期、必要目標完成日期、延後天數與重複扣款測試。
