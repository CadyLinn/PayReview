...
3. 使用者必須以 Email 或 Google 帳號登入後才能使用核心功能。
4. 試算在確認前不修改正式財務資料。
...
| 帳號 | Firebase Authentication | Email、Google 登入與帳號生命週期 |
...
使用者必須使用 Email 或 Google 帳號登入後才能進入首次設定與核心功能。不使用 Firebase Anonymous Authentication，也不提供訪客模式。

Firebase Authentication session 是 MVP 的帳號存取邊界。Security Rules 只允許已登入使用者讀寫自己的 `users/{uid}` 路徑；App 在 Authentication 狀態解析為已登入後即可開始首次設定，不需要另行呼叫 Cloud Function。
...
3. 以穩定的 deletion request ID 呼叫受信任的 Cloud Function。Function 驗證 Firebase token 的 `auth_time` 符合近期重新驗證門檻，建立 server-owned deletion job，排入可持續重試的後端佇列，並只在以上操作全部持久化後回傳高熵度 opaque receipt。此流程尚未實作，不得在未補齊伺服器端刪除保護前對外發布帳號刪除功能。
...
6. Firebase Authentication 刪除確認後，保留 deletion job 的最小化狀態與 request ID hash 至少七天，供後端重試與稽核。所有新帳號必須由 Firebase Authentication 產生新 `uid`，禁止重用已刪除 `uid`。Opaque receipt 查詢只回傳 job status、不回傳個人或財務資料，七天後失效；receipt 遺失不影響後端繼續刪除。
...
- Security Rules 必須只允許已登入使用者讀寫自己的 `users/{uid}` 路徑；不得使用公開讀寫規則。
- 不得使用公開讀寫規則。
...
- Firestore 離線 enqueue 可以更新 pending UI，但快取不構成伺服器確認。恢復連線時由 Security Rules 驗證 Firebase Authentication；若伺服器拒絕寫入，Repository 必須回滾 optimistic state、標示該操作未保存並提供重試或修正入口。
- 使用 Firebase Emulator 測試跨使用者隔離、拒絕未授權寫入與資料驗證。
- 使用 Emulator 測試未登入與跨使用者存取都會被拒絕，並測試離線 pending-write 重送的失敗回滾。
...
2. 在 Firebase Authentication 啟用 Email/密碼與 Google 登入。
